...
public class GuiPage extends GuiBooklet {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int mouseButton) {
        ...
        for (ItemDisplay display : this.itemDisplays) {
            display.onMousePress(mouseButton, (int)mouseX, (int)mouseY);
        }
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int state) {
        boolean flag = super.mouseReleased(mouseX, mouseY, state);
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int clickedMouseButton, double dragX, double dragY) {
        boolean flag = super.mouseDragged(mouseX, mouseY, clickedMouseButton, dragX, dragY);

        for (IBookletPage page : this.pages) {
            if (page != null) {
                page.mouseDragged(this, mouseX, mouseY, clickedMouseButton, dragX, dragY);
            }
        }
 ...
 */
    ...
    public void init() {
        ...
        if (links != null && !links.isEmpty()) {
            this.buttonViewOnline = new TexturedButton(RES_LOC_GADGETS, this.guiLeft + this.xSize - 24, this.guiTop + this.ySize - 25, 0, 172, 16, 16, Collections.singletonList(TextFormatting.GOLD + StringUtil.localize("booklet." + ActuallyAdditions.MODID + ".onlineButton")), btn -> {
                this.minecraft.setScreen(new ConfirmOpenLinkScreen((yes) -> {
                    if (yes) {
                        for (String link : links) {
    ...
    public void drawScreenPre(MatrixStack matrices, int mouseX, int mouseY, float partialTicks) {
        ...
        for (ItemDisplay display : this.itemDisplays) {
            display.drawPre(matrices);
        }
    }

    @Override
    public void drawScreenPost(MatrixStack matrices, int mouseX, int mouseY, float partialTicks) {
        super.drawScreenPost(matrices, mouseX, mouseY, partialTicks);
        ...
        for (ItemDisplay display : this.itemDisplays) {
            display.drawPost(matrices, mouseX, mouseY);
        }
    }
