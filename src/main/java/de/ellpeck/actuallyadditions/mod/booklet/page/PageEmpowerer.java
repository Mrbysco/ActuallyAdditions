...
public class PageEmpowerer extends BookletPage {
    ...
    public PageEmpowerer(int localizationKey, IItemProvider itemProvider) {
    ...
    private static EmpowererRecipe getRecipe(IItemProvider provider) {
        return getRecipe(new ItemStack(provider));
    }

    private static EmpowererRecipe getRecipe(ItemStack stack) {
        Minecraft mc = Minecraft.getInstance();
        World world = mc.level;
        if (world != null) {
            List<EmpowererRecipe> matchingRecipes = world.getRecipeManager().getAllRecipesFor(ActuallyRecipes.Types.EMPOWERING).stream()
