...
public class BookletEntry implements IBookletEntry {
    ...
    private static boolean fitsFilter(IBookletPage page, String searchBarText) {
        ...
        if (!items.isEmpty()) {
            for (ItemStack stack : items) {
                if (StackUtil.isValid(stack)) {
                    List<ITextComponent> tooltip = stack.getTooltipLines(mc.player, mc.options.advancedItemTooltips
                        ? ITooltipFlag.TooltipFlags.ADVANCED
                        : ITooltipFlag.TooltipFlags.NORMAL);
                    for (ITextComponent strg : tooltip) {
                        if (strg != null && strg.getString().toLowerCase(Locale.ROOT).contains(searchBarText)) { //TODO: Check if this works
                            return true;
                        }
                    }
                }
        ...
        if (!fluids.isEmpty()) {
            for (FluidStack stack : fluids) {
                if (stack != null) {
                    String strg = stack.getDisplayName().getString(); //TODO: Check if this works
    ...
    @Override
    @OnlyIn(Dist.CLIENT)
    public ITextComponent getLocalizedNameWithFormatting() {
        return new TranslationTextComponent(this.getLocalizedName()).withStyle(this.color);
    }
