...
public class GuiMainPage extends GuiBooklet {
    ...
    private IFormattableTextComponent quote;
    ...
    public void init() {
        ...
        if (quoteSplit.length == 2) {
            ...
            this.quote = new StringTextComponent(quoteSplit[0]).withStyle(TextFormatting.DARK_GRAY).withStyle(TextFormatting.ITALIC)
                    .append("\n - ")
                    .append(new StringTextComponent(quoteSplit[1]).withStyle(TextFormatting.BLACK));
        ...
        configText.add(TextFormatting.GOLD + StringUtil.localize("booklet." + ActuallyAdditions.MODID + ".configButton"));
        ...
        achievementText.add(TextFormatting.GOLD + StringUtil.localize("booklet." + ActuallyAdditions.MODID + ".achievementButton"));
        ...
        if (!data.didBookTutorial) {
            ...
            this.tutorialButton = new Button(this.guiLeft + 140 / 2 - 50, this.guiTop + 146, 100, 20, new StringTextComponent("Please click me <3"),
                    (button) -> {
                        if (this.hasBookmarkButtons()) {
                            ...
                            PlayerSave playerData = PlayerData.getDataFromPlayer(this.getMinecraft().player);
                            playerData.didBookTutorial = true;
        ...
        for (int i = 0; i < BUTTONS_PER_PAGE; i++) {
            ...
            if (displayed.size() > i) {
                ...
                this.addButton(new EntryButton(this, i, this.guiLeft + 156, this.guiTop + 11 + i * 13, 115, 10,
                        new StringTextComponent("- ").append(entry.getLocalizedNameWithFormatting()), ItemStack.EMPTY, (button) -> {
                    if (button instanceof EntryButton) {
                        int entryID = ((EntryButton)button).getEntryID();
                        if(displayed.size() > entryID) {
                            IBookletEntry buttonEntry = displayed.get(entryID);
                            if (buttonEntry != null) {
                                this.getMinecraft().setScreen(new GuiEntry(this.previousScreen, this, buttonEntry, 0, "", false));
                            }
                        }
                    }
                }));
...
/*
    ...
    */
    ...
    public void drawScreenPre(MatrixStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        } else if (this.quote != null && this.quote != null && this.quoteGuy != null) {
...
//            }
            this.font.drawWordWrap(this.quote, this.guiLeft + 12, this.guiTop + 63, 120, 100);
