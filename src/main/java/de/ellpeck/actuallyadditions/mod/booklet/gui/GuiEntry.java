        ...
        for (int x = 0; x < 2; x++) {
            for (int y = 0; y < BUTTONS_PER_PAGE; y++) {
                ...
                if (this.chapters.size() > id + idOffset) {
                    ...
                    this.addButton(new EntryButton(this, id, this.guiLeft + 14 + x * 142, this.guiTop + 11 + y * 13, 115, 10,
                            chapter.getLocalizedNameWithFormatting(), chapter.getDisplayItemStack(), (button) -> {
                        if (button instanceof EntryButton) {
                            int actualId = ((EntryButton)button).getEntryID() + this.entryPage * BUTTONS_PER_PAGE * 2;

                            if (this.chapters.size() > actualId) {
                                IBookletChapter actualChapter = this.chapters.get(actualId);
                                if (actualChapter != null) {
                                    IBookletPage[] pages = actualChapter.getAllPages();
