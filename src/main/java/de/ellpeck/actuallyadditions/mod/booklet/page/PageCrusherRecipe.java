...
public class PageCrusherRecipe extends BookletPage {
    ...
    public PageCrusherRecipe(int localizationKey, IItemProvider itemProvider) {
    ...
    private static CrushingRecipe getRecipe(IItemProvider provider) {
        return getRecipe(new ItemStack(provider));
    }

    private static CrushingRecipe getRecipe(ItemStack stack) {
        Minecraft mc = Minecraft.getInstance();
        World world = mc.level;
        if (world != null) {
            List<CrushingRecipe> matchingRecipes = world.getRecipeManager().getAllRecipesFor(ActuallyRecipes.Types.CRUSHING).stream()
